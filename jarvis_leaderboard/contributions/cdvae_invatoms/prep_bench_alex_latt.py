...
df=pd.read_csv('AI-AtomGen-Tc-alex_supercon-test-rmse.csv.zip')
mem={}
test_ids=[]
f_a=open('AI-AtomGen-Tc_a-alex_supercon-test-mae.csv','w')
f_b=open('AI-AtomGen-Tc_b-alex_supercon-test-mae.csv','w')
f_c=open('AI-AtomGen-Tc_c-alex_supercon-test-mae.csv','w')
f_alpha=open('AI-AtomGen-Tc_alpha-alex_supercon-test-mae.csv','w')
f_beta=open('AI-AtomGen-Tc_beta-alex_supercon-test-mae.csv','w')
f_gamma=open('AI-AtomGen-Tc_gamma-alex_supercon-test-mae.csv','w')
...
for i in d:
    if i['id'] not in test_ids and i['Tc']!='na':
        train_a[i['id']]=Atoms.from_dict(i['atoms']).lattice.abc[0]
        train_b[i['id']]=Atoms.from_dict(i['atoms']).lattice.abc[1]
        train_c[i['id']]=Atoms.from_dict(i['atoms']).lattice.abc[2]
        train_alpha[i['id']]=Atoms.from_dict(i['atoms']).lattice.angles[0]
        train_beta[i['id']]=Atoms.from_dict(i['atoms']).lattice.angles[1]
        train_gamma[i['id']]=Atoms.from_dict(i['atoms']).lattice.angles[2]
info={}
info['test']=mem_a
info['train']=train_a
print('train',len(train_a))
dumpjson(data=info,filename='alex_supercon_Tc_a.json')
info={}
info['test']=mem_b
info['train']=train_b
print('train',len(train_b))
dumpjson(data=info,filename='alex_supercon_Tc_b.json')
info={}
info['test']=mem_c
info['train']=train_c
print('train',len(train_c))
dumpjson(data=info,filename='alex_supercon_Tc_c.json')
info={}
info['test']=mem_alpha
info['train']=train_alpha
print('train',len(train_alpha))
dumpjson(data=info,filename='alex_supercon_Tc_alpha.json')
info={}
info['test']=mem_beta
info['train']=train_beta
print('train',len(train_beta))
dumpjson(data=info,filename='alex_supercon_Tc_beta.json')
info={}
info['test']=mem_gamma
info['train']=train_gamma
print('train',len(train_gamma))
dumpjson(data=info,filename='alex_supercon_Tc_gamma.json')
import glob,os
